...
def register():
    if request.method=='GET':
        ...
        if(password1 != password):
            ...
            if len(result) == 0:
                return u'没有这个用户了'
                sql = "INSERT INTO STUDENT(STU_NO, PASSWORD) VALUES ('%s', '%s')" % (stu_id, password)
            else:
                if result[0][6] == user:
...
def recommed():
    question='中加201'
    sql = "SELECT * FROM courses WHERE class='%s'" % question
    result = query.query(sql)
    sql2='SELECT course_id FROM selected_course WHERE STU_NO=20416301;'
    result2 = query.query(sql2)
    #在下一条代码中对result进行一些操作使得courses变成这样的字典：
    # courses = [
    #     {'course_id': 1, 'course_name': '课程1', 'credits': '3', ......},
    # ]
    # print(result)
    recommended_courses = []
    selected_courses = []
    print(result2)
    flatten_result = tuple(item for sublist in result2 for item in sublist)
    for row in result:
        course = {
            'course_id': row[0],
            'course_name': row[1],
            ...
            'instructor': row[3],
            'class': row[4],
            'teaching_hours': row[5],
            'class_time': row[6]
        }
        print(row[0])
        if(row[0] in flatten_result):
    ...
    for course in selected_courses:
        course_row = f"""
            <tr style="border: 1px solid black;">
                ...
                <td style="border: 1px solid black;">{course['credits']}</td>
                <td style="border: 1px solid black;">{course['instructor']}</td>
                <td style="border: 1px solid black;">{course['class']}</td>
                <td style="border: 1px solid black;">{course['teaching_hours']}</td>
                <td style="border: 1px solid black;">{course['class_time']}</td>
                <td style="border: 1px solid black;"><button id="dropCourseButton{ course['course_id'] }" onclick="dropCourse({ course['course_id'] })">退课</button></td>
            </tr>
        """
        selected_result += course_row
    ...
    for course in recommended_courses:
        course_row = f"""
            <tr style="border: 1px solid black;">
                ...
                <td style="border: 1px solid black;">{course['teaching_hours']}</td>
                <td style="border: 1px solid black;">{course['class_time']}</td>
                <td style="border: 1px solid black;"><button id="selectCourseButton{course['course_id']}" onclick="selectCourse({course['course_id']})">选课</button></td>
            </tr>
        """
        recommended_result += course_row

    return render_template('recommed.html', selected_result=selected_result, recommended_result=recommended_result)
...
def drop_course():
    ...
    course_name='你的课'
    # 执行退课操作
    sql = 'DELETE FROM selected_course WHERE COURSE_ID=%s;' % (course_id)
    result = query.query(sql)
    # 构建响应数据
    response_data = {
        'message': f'Successfully dropped course: {course_name} ({course_id})'
    }
...
def select_course():
    ...
    print(course_id)
    # 执行退课操作
    sql = 'INSERT INTO selected_course (stu_no, course_id) VALUES (%s, %s);' % (studentnumber,course_id)
    result = query.query(sql)
    # 构建响应数据
    response_data = {
        'message': f'Successfully select course: {course_id}'
    }
    return jsonify(response_data)
